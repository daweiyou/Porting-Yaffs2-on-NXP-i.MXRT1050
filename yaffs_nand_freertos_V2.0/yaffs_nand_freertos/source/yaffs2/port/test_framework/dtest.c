...
void copy_in_a_file(const char *yaffsName,const char *inName)
{
	...
	int ni,no;
	inh = yaffs_open(inName,O_RDONLY, 0);
...
int long_test(int argc, char *argv[])
{
	...
	char str[100];
	char filename[255] = {0};
	char filename_tmp1[255] = {0};
	char filename_tmp2[255] = {0};
	char filename_tmp3[255] = {0};
...
//	dumpDir("/flash");
	...
	yaffs_mount(NXP_YAFFS_NAME);
	printf("\nDirectory look-up of %s\n",NXP_YAFFS_NAME);
	dumpDir(NXP_YAFFS_NAME);
...
//	printf("open /boot/b1 O_CREAT, f=%d\n",f);
    ...
    memcpy(filename,NXP_YAFFS_NAME,sizeof(NXP_YAFFS_NAME));
    strcat(filename,"/b1");

    
	f = yaffs_open(filename, O_RDONLY,0);

	printf("open %s readonly, f=%d\n",filename, f);

    if(f < 0)
    {
    	f = yaffs_open(filename, O_CREAT,S_IREAD | S_IWRITE);
    
    	printf("open %s O_CREAT, f=%d\n",filename, f);
	}
	if(f < 0)
	{
        error_trap("create file fail");
	}
	...
	f = yaffs_open(filename, O_RDWR,0);
...
//	copy_in_a_file("/boot/file with a long name","xxx");
    ...
    memcpy(filename_tmp1,NXP_YAFFS_NAME,sizeof(NXP_YAFFS_NAME));
    strcat(filename_tmp1,"/b2");
	copy_in_a_file(filename_tmp1,filename);

    memcpy(filename_tmp2,NXP_YAFFS_NAME,sizeof(NXP_YAFFS_NAME));
    strcat(filename_tmp2,"/file with a long name");
	copy_in_a_file(filename_tmp2,filename);
	...
	// Check stat
	r = yaffs_lstat(filename_tmp2,&ystat);

    memcpy(filename_tmp3,NXP_YAFFS_NAME,sizeof(NXP_YAFFS_NAME));
    strcat(filename_tmp3,"/r1");

	// Check rename

	r = yaffs_rename(filename_tmp2,filename_tmp3);
	...
	// Check mkdir
    memcpy(directory_name,NXP_YAFFS_NAME,sizeof(NXP_YAFFS_NAME));
    strcat(directory_name,"/directory1");

	r = yaffs_mkdir(directory_name,0);

	printf("\nDirectory look-up of %s\n",NXP_YAFFS_NAME);
	dumpDir(NXP_YAFFS_NAME);

	printf("\nDirectory look-up of %s\r\n",directory_name);
	dumpDir(directory_name);

    memcpy(filename_tmp1,directory_name,sizeof(directory_name));
    strcat(filename_tmp1,"/file with a long name");
